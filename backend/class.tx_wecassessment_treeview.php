...
require_once(PATH_t3lib.'class.t3lib_treeview.php');
	 ...
	 *
	 */
class tx_wecassessment_tceFunc_selectTreeView extends t3lib_treeview {
			...
			if ($row['sys_language_uid'] && $row['l18n_parent'] && ($table == 'tt_news' || $table == 'tx_wecassessment_category')) { // the current record is a translation of another record
				...
				if($config['treeView'] AND $config['foreign_table']) {
					...
					if($config['treeViewClass'] AND is_object($treeViewObj = &t3lib_div::getUserObj($config['treeViewClass'],'user_',false)))      {
						...
						$treeViewObj = t3lib_div::makeInstance('tx_wecassessment_tceFunc_selectTreeView');
					}

					if ($this->excludeList) {
						$catlistWhere = ' AND tx_wecassessment_category.uid NOT IN ('.implode(t3lib_div::intExplode(',',$this->excludeList),',').')';
					}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/wec_assessment/backend/class.tx_wecassessment_treeview.php'])    {
    include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/wec_assessment/backend/class.tx_wecassessment_treeview.php']);
}
?>
